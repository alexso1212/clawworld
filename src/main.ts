...
function staticMockResource(resourceId: ResourcePartitionId): OpenClawSnapshot['resources'][number] | null {
  return staticMockSnapshot.resources.find((resource) => resource.id === resourceId) ?? null;
}
...
async function loadResourceDetail(targetResourceId: ResourcePartitionId): Promise<void> {
  ...
  const request = (async () => {
    if (useStaticMockData) {
      const resource = staticMockResource(targetResourceId);
      if (!resource) {
        throw new Error('resource detail unavailable');
      }
      resourceDetailItemsById.set(targetResourceId, resource.items ?? []);
...
async function openFolderPath(item: OpenClawResourceItem): Promise<void> {
  if (useStaticMockData) {
    setModalFeedback(uiLocale === 'zh' ? `静态演示暂不支持打开目录 · ${item.title}` : `Open Folder is unavailable in the static demo · ${item.title}`, 'error');
    return;
  }
...
async function openPreviewForItem(item: OpenClawResourceItem): Promise<void> {
  if (useStaticMockData) {
    setModalFeedback(uiLocale === 'zh' ? `静态演示暂不支持文件预览 · ${item.title}` : `Preview is unavailable in the static demo · ${item.title}`, 'error');
    return;
  }
...
async function refreshTelemetry(): Promise<void> {
  if (useStaticMockData) {
    lastSnapshot = staticMockSnapshot;
    const activeScene = getActiveScene();

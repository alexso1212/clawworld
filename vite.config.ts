...
// @ts-expect-error local .mjs helper has no first-party type package
...
async function refreshLiveDetail(cacheKey: string, resourceIds: string[]): Promise<CachedSnapshot> {
  ...
  const request = createOpenClawSnapshot({
    ...
    .then(async (snapshot: CachedSnapshot) => {
      cachedLiveDetailByKey.set(cacheKey, snapshot);
...
function telemetryMiddleware() {
  return async (req: IncomingMessage, res: ServerResponse<IncomingMessage>, next: Connect.NextFunction) => {
    ...
    try {
      ...
      res.end(JSON.stringify(wantsMock ? snapshot : {
        ...snapshot,
        resources: snapshot.resources.map(({ items, ...resource }: CachedSnapshot['resources'][number]) => resource)
      }));
    } catch (error) {
      res.statusCode = 500;
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
